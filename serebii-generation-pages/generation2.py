...
def scrape_page(url):
    ...
    if soup is not None: # check for null
        ...
        ## tr_tags MAP ##
        # tr_tags[0]: the label for label for name, other names, numbers, gender ratio, type
        ...
        # tr_tags[6]: national number (embedded table)
        # tr_tags[7]: johto number (embedded table)
        # tr_tags[8]: male ratio(embedded table)
        # tr_tags[9]: female ratio (embedded table)
        # tr_tags[10]: labels for classification, height, weight, capture rate, and base egg steps
        # tr_tags[11]: data for the above
        ...
        if found_data:
            ...
            ### DEX NUMBER DATA ###
            inner_table_td_tags = td_tags[2].find_all('td')[1::2]
            entry['National Dex Number'] = int(re.sub(r'\D', '', inner_table_td_tags[0].text.strip())) # strips out everything that would make parsing as int fail
            entry['Hoenn Dex Number'] = int(re.sub(r'\D', '', inner_table_td_tags[1].text.strip())) # strips out everything that would make parsing as int fail

            ### GENDER RATIO DATA ###
            inner_table_td_tags = td_tags[3].find_all('td')[1::2]
            entry['Male Ratio'] = float(re.sub(r'[^\d.]', '', inner_table_td_tags[0].text.strip()))
            entry['Female Ratio'] = float(re.sub(r'[^\d.]', '', inner_table_td_tags[1].text.strip()))
        ...
        if found_data:
            ...
            ### BASE EGG STEPS DATA ###
            entry['Base Egg Steps'] = int(re.sub(r'\D', '', td_tags[4].text))
        else:
            # need the name here or else theres no way to save the data properly
        ...
        if found_data:
            ...
            matches = re.findall(pattern, td_tags[2].text)
        ...
        if found_data:
            ...
            entry['Wild Hold Items'] = wild_hold_item_parse(td_tags[0].text)

            ### EGG GROUPS ###
            entry['Egg Groups'] = []
            for tr_tag in td_tags[1].find_all('tr'):
                entry['Egg Groups'].append(tr_tag.find('a').text.strip())
        else:
            logger.warning("Failed to find wild hold item, egg group data...")
            found_data = True
        ...
        try:
            td_tags = tr_tags[2].find_all('td') # the columns in the target row
        except Exception:
            found_data = False
